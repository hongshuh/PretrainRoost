...
from roost.core_pretrain_fl_mml import Normalizer, RobustL1Loss, RobustL2Loss, sampled_softmax, BarlowTwinsLoss,BT_Magpie
    ...
    for name, task in task_dict.items():
        ...
        if task == "SSL":
            if loss_dict[name] == "SSL":
                criterion_dict[name] = (task, BT_Magpie("cpu",batch_size,embed_size,lambda_))
            if loss_dict[name] == 'SSL_magpie':
