...
class CompositionData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # magpie_feature = self.magpie.iloc[idx].values
        # magpie_feature = magpie_feature.astype(np.float32)
        # # print(type(magpie_feature))
        # magpie_feature = torch.Tensor(magpie_feature)
        # print(magpie_feature.shape)
        # exit()
        embeddings = self.embeddings.get(self.df['material_id'][idx])
...
def collate_batch(dataset_list):
    ...
    for i, (inputs, target, *cry_ids) in enumerate(dataset_list):
        ...
        cry_base_idx += n_i
    # print('collate batch',len(batch_targets))
