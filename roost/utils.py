...
from roost.core_pretrain import Normalizer, RobustL1Loss, RobustL2Loss, sampled_softmax, BarlowTwinsLoss
...
def init_model(
    ...
    elem_emb_len = model_params["elem_emb_len"]
    task_dict = model_params["task_dict"]
    ...
    if fine_tune is not None:
        ...
        model = model_class( robust = robust, n_targets = n_targets,
                elem_emb_len = elem_emb_len, task_dict = task_dict,
            device=device,
        )
        model.to(device)
        model.load_state_dict(torch.load(fine_tune, map_location=device),strict=False)
